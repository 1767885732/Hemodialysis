...
{
    public partial class Item_Weight : ViewBase
    {
        #region 类变量
        ...
        public Item_Weight(string CurrentPatientHemoId)
        {
            InitializeComponent();
        ...
        public override void InzationData()
        {
            using (var _worker = new BackgroundWorker())
            {
                ...
                {
                    _recipeDataTable = objDataReport.GetDataReportPatientRecipeList(_currentPatientHemoId, "4");
        ...
        public override void GetVascualToUpLoad(string resultInfo)
        {
            ...
            foreach (DataReportModel.MED_HEMO_RECIPERow row in dtSource.Rows)
            {
                if (row.ISUPLOAD == "1")
                {
                    ...
                    //去进行上传操作                   
                    ReportResult result = reporter.ReportXTGTZ(new XTGTZEntity(resultInfo, row.RECIPE_DATE) { QK = "非卧床", GTZZ = row.DRY_WEIGHT });
                    if (result.Success)
                    {
