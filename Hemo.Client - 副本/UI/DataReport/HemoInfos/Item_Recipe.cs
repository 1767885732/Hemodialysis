...
{
    public partial class Item_Recipe : ViewBase
    {
        #region 类变量

        private DataReportModel.MED_HEMO_RECIPEDataTable _recipeDataTable;
        ...
        public Item_Recipe(string CurrentPatientHemoId)
        {
            InitializeComponent();
        ...
        {
            var dr = gridView4.GetFocusedDataRow() as DataReportModel.MED_HEMO_RECIPERow;
            if (dr == null)
                return;
            if (e.Clicks == 1)
            {
        ...
        private void gridView4_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                var curRow = (DataReportModel.MED_HEMO_RECIPERow)gridView4.GetDataRow(e.RowHandle);
                if (curRow == null)
                    return;
        ...
        public override void InzationData()
        {
            ...
            {
                _recipeDataTable = new DataReportModel.MED_HEMO_RECIPEDataTable();
                _worker.DoWork += delegate(object sender, DoWorkEventArgs e)
                {
                    _recipeDataTable = objDataReport.GetDataReportPatientRecipeList(_currentPatientHemoId, "2");

                };
                _worker.RunWorkerCompleted += delegate(object sender1, RunWorkerCompletedEventArgs r1)
                {
                    this.gridControl1.DataSource = _recipeDataTable;
                };
        ...
        {
            var dtSource = ((System.Data.DataView)(this.gridView4.DataSource)).Table as DataReportModel.MED_HEMO_RECIPEDataTable;
            ...
            foreach (DataReportModel.MED_HEMO_RECIPERow row in dtSource.Rows)
            {
                if (row.ISUPLOAD == "1")
                {

                    //去进行上传操作                   
                    ReportResult result = reporter.ReportXTTXCF(new XTTXCFEntity(resultInfo, row.RECIPE_DATE, row.FREQUENCY_TIMES.ToString(), row.FREQUENCY_HOURS.ToString(), new List<string> { "透析浓缩A液", "透析浓缩B液" }, "国产", "低通量", "一次性使用", new List<string> { row.FIRST_PURIFIER_NAMESTR }, row.FIRST_PURIFIER_M2INDEX) { HDFCS = "1次/2周", HDFZLSJ = "4", HPCS = "其它", HPZLSJ = "2", TXNSAY = "商品化成品", JLZND = "2.0 mmol/L", GLZND = "1.5 mmol/L", HT = "否", AJS = "否", TXNSBY = "商品化成品" });
                    if (result.Success)
                    {
