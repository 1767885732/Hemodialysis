...
{
    public partial class EditCKDMBD : HemoBaseFrm
    {
        #region 类变量
        ...
        public EditCKDMBD(HemodialysisModel.MED_ANEMIA_CKDMBD_ASSESSDataTable pdt, HemodialysisModel.MED_ANEMIA_CKDMBD_ASSESSRow prow)
        {
            InitializeComponent();
            dt = pdt;
            row = prow;
        ...
        {
            row.GAI = Utility.CDouble(txtGAI.Text);
            row.LIN = Utility.CDouble(txtLIN.Text);
            row.JXLSM = Utility.CDouble(txtJXLSM.Text);
            row.JZXJS = Utility.CDouble(txtJZXJS.Text);
            try
            {
        ...
        private void EditCKDMBD_Load(object sender, EventArgs e)
        {
            ...
            if (row == null)
            {
                ...
                row.ASSESS_TYPE = "CKDMBD";
                row.CREATE_DATE = System.DateTime.Now;
                dt.AddMED_ANEMIA_CKDMBD_ASSESSRow(row);
            }
            else
            {
                txtGAI.Text = row.GAI.ToString();
                txtLIN.Text = row.LIN.ToString();
                txtJXLSM.Text = row.JXLSM.ToString();
                txtJZXJS.Text = row.JZXJS.ToString();
            }
        }
