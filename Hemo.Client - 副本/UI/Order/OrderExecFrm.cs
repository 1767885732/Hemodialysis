...
namespace Hemo.Client.UI.Order
{
    public partial class OrderExecFrm :HemoBaseFrm
    {
        ...
        private HemodialysisModel.MED_CURE_MAINRow _cureMainRow;
        ...
        public OrderExecFrm(DateTime enterDate, PatientModel.MED_PATIENTSRow patientRow, HemodialysisModel.MED_CURE_MAINRow cureMainRow)
        {
            this.InitializeComponent();

            this._enterDate = enterDate;
            this._patientRow = patientRow;
            this._cureMainRow = cureMainRow;
        }
        ...
        {
            this._orderDataTable = this._orderService.GetOrderList(this._patientRow.PATIENT_ID, this.BeginEnterDate, this.EndEnterDate);
            //报错，暂时注释掉
        ...
        private void btnExecOrder_Click(object sender, EventArgs e)
        {
            ...
            {
                OrderModel.MED_DRUG_EXECDataTable drugExecDataTable = new OrderModel.MED_DRUG_EXECDataTable();

                foreach (var orderRow in orderRows)
                {
                    OrderModel.MED_DRUG_EXECRow drugExecRow = drugExecDataTable.NewMED_DRUG_EXECRow();

                    drugExecRow.DRUG_EXEC_ID = Guid.NewGuid().ToString();
                    drugExecRow.NURSE = LoginUser.User.USER_NAME;
                    drugExecRow.EXCUTE_DATE = DateTime.Now;
                    drugExecRow.DRUG_ORDER_ID = orderRow.ORDER_NO;
                    drugExecRow.CURE_ID = this._cureMainRow.CURE_ID;

                    drugExecDataTable.AddMED_DRUG_EXECRow(drugExecRow);
                }

                this._orderService.SaveDrugExecInfo(drugExecDataTable);
