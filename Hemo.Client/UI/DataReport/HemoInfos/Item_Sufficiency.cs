...
{
    public partial class Item_Sufficiency : ViewBase
    {
        #region 类变量

        private DataReportModel.MED_ESTIMATE_SUFFICIENCYDataTable _sufficiencyDataTable;
        ...
        public Item_Sufficiency(string CurrentPatientHemoId)
        {
            InitializeComponent();
        ...
        {
            var dr = gridView4.GetFocusedDataRow() as DataReportModel.MED_ESTIMATE_SUFFICIENCYRow;
            if (dr == null)
                return;
            if (e.Clicks == 1)
            {
        ...
        private void gridView4_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                var curRow = (DataReportModel.MED_ESTIMATE_SUFFICIENCYRow)gridView4.GetDataRow(e.RowHandle);
                if (curRow == null)
                    return;
        ...
        public override void InzationData()
        {
            ...
            {
                _sufficiencyDataTable = new DataReportModel.MED_ESTIMATE_SUFFICIENCYDataTable();
                _worker.DoWork += delegate(object sender, DoWorkEventArgs e)
                {
                    _sufficiencyDataTable = objDataReport.GetDataReportPatientEstimateSufficiencyList(_currentPatientHemoId);

                };
                _worker.RunWorkerCompleted += delegate(object sender1, RunWorkerCompletedEventArgs r1)
                {
                    this.gridControl1.DataSource = _sufficiencyDataTable;
                };
        ...
        {
            var dtSource = ((System.Data.DataView)(this.gridView4.DataSource)).Table as DataReportModel.MED_ESTIMATE_SUFFICIENCYDataTable;
            ...
            foreach (DataReportModel.MED_ESTIMATE_SUFFICIENCYRow row in dtSource.Rows)
            {
                if (row.ISUPLOAD == "1")
                {

                    //去进行上传操作                   
                    ReportResult result = reporter.ReportXTCFX(new XTCFXEntity(resultInfo, row.CREATE_DATE, Convert.ToString(row.TS / 100), row.WEIGHT.ToString()) { BMI = row.TIBC.ToString(), BMTJ = row.SI.ToString(), TQNS = row.BEFORE_BLOODUREA.ToString(), THNS = row.AFTER_BLOODUREA.ToString(), TXSJ = row.TIME.ToString(), CLL = row.UF.ToString(), URR = row.URR.ToString(), SPKTV = row.SPKT_V.ToString() });
                    if (result.Success)
                    {
