...
{
    public partial class Item_Vasular : ViewBase
    {
        #region 类变量

        private DataReportModel.MED_VASCULAR_ACCESSDataTable _vascularDataTable;
        ...
        public Item_Vasular(string CurrentPatientHemoId)
        {
            InitializeComponent();
        ...
        {
            var dr = gridView4.GetFocusedDataRow() as DataReportModel.MED_VASCULAR_ACCESSRow;
            if (dr == null)
                return;
            if (e.Clicks == 1)
            {
        ...
        private void gridView4_CustomRowCellEdit(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                var curRow = (DataReportModel.MED_VASCULAR_ACCESSRow)gridView4.GetDataRow(e.RowHandle);
                if (curRow == null)
                    return;
        ...
        public override void InzationData()
        {
            ...
            {
                _vascularDataTable = new DataReportModel.MED_VASCULAR_ACCESSDataTable();
                _worker.DoWork += delegate(object sender, DoWorkEventArgs e)
                {
                    _vascularDataTable = objDataReport.GetDataReportPatientVascularList(_currentPatientHemoId);

                };
                _worker.RunWorkerCompleted += delegate(object sender1, RunWorkerCompletedEventArgs r1)
                {
                    this.gridControl1.DataSource = _vascularDataTable;
                };
        ...
        {
            var dtSource = ((System.Data.DataView)(this.gridView4.DataSource)).Table as DataReportModel.MED_VASCULAR_ACCESSDataTable;
            ...
            foreach (DataReportModel.MED_VASCULAR_ACCESSRow row in dtSource.Rows)
            {
                if (row.ISUPLOAD == "1")
                {

                    //去进行上传操作                   
                    ReportResult result = reporter.ReportXTXGTL(new XTXGTLEntity(resultInfo, DateTime.Now, row.VASCULAR_ACCESS_TYPENAME, row.LATERAL_POSITIONNAME == "左侧" ? "左" : "右") { XGTLWZA = row.VASCULAR_POSTIONNAME.Contains("静脉") ? row.VASCULAR_POSTIONNAME : string.Empty, ZXJMZGFF = "超声介入", XGTLWZB = !row.VASCULAR_POSTIONNAME.Contains("静脉") ? row.VASCULAR_POSTIONNAME : string.Empty });
                    if (result.Success)
                    {
                        ...
                        rowExtend.MAPIP = row.VASCULAR_ACCESS_ID;
