...
{
    public partial class Item_Infection : ViewBase
    {
        #region 类变量
        ...
        public Item_Infection(string CurrentPatientHemoId)
        {
            InitializeComponent();
        ...
        public override void InzationData()
        {
            using (var _worker = new BackgroundWorker())
            {
                ...
                {
                    _patientDiagDataTable = objDataReport.GetDataReportPatientDiagnoseList(_currentPatientHemoId, "0");
        ...
        public override void GetVascualToUpLoad(string resultInfo)
        {
            ...
            foreach (DataReportModel.MED_PATIENTSRow row in dtSource.Rows)
            {
                if (row.ISUPLOAD == "1")
                {
                    ...
                    string[] strl = row.INFECTIOUS_CHECK_RESULT.Split(',');

                    var crbList = new List<string>();

                    foreach (string s in strl)
                    {
                        crbList.Add(s.ToString());

                    }

                    //去进行上传操作                   
                    ReportResult result = reporter.ReportZDCRB(new ZDCRBEntity(resultInfo, row.CREATE_DATE, crbList) { QTQSM = string.Empty });
                    if (result.Success)
                    {
