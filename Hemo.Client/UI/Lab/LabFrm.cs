...
namespace Hemo.Client.UI.Lab
{
    ...
    public partial class LabFrm : HemoBaseFrm {
        #region 变量
        ...
        public LabFrm(PatientModel.MED_PATIENTSRow patientRow) {
            ...
            using (BackgroundWorker worker = new BackgroundWorker()) {
                worker.DoWork += (o1, e1) => {
                    try {
                        ...
                        string error = InterfaceUtility.SynchronizeLabDatasByPatiets(this._patientRow.PATIENT_ID, interFaceUrl);
        ...
        private void loadLabList() {
            ...
            //if (_dtPatientLab != null && _dtPatientLab.Rows.Count > 0) {
            //    gcLabMain.DataSource = _dtPatientLab;
            //}
            //else {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            else if (LabReportType.Equals("0"))
            {
                ...
                foreach (var item in query.ToList())
                {
                    ...
                    newRow.RESULT_DATE = item.RESULT_DATE;
        ...
        {
            DataTable dtLabData = gcLabDetail.DataSource as DataTable;

            if (dtLabData != null && dtLabData.Rows.Count > 0)
            {
                ExcelNPOIHelper exOut = new ExcelNPOIHelper();
                DataSet ds = new DataSet();
                ds.Tables.Add(dtLabData);
                exOut.DS_Main = ds;
                // exOut.ExcelTempleteName = "";
                IList<ColoumSetting> settingList = new List<ColoumSetting>();
                settingList.Add(new ColoumSetting { CellNum = 0, ColumnName = "REPORT_ITEM_NAME" });
                settingList.Add(new ColoumSetting { CellNum = 1, ColumnName = "RESULT" });
                settingList.Add(new ColoumSetting { CellNum = 2, ColumnName = "UNITS" });
                settingList.Add(new ColoumSetting { CellNum = 3, ColumnName = "REPORT_ITEM_NAME" });
                // exOut.ExportByCustomerDirectMode();
            }
        }
