...
{
    [ToolboxItem(true)]
    public partial class PatientMaterialQueryUI : ViewBase
    {
        #region 变量
        private IPatient patientService = ServiceManager.Instance.PatientService;
        private DataTable MaterDetail = new DataTable();

        #endregion
        /// <summary>
        /// 构造函数
        /// </summary>
        public PatientMaterialQueryUI()
        {
            InitializeComponent();
            if (!this.DesignMode)
            {
        ...
        {
            var MaterDt = new DataTable();
            using (BackgroundWorker worker = new BackgroundWorker())
            {

                worker.DoWork += delegate(object sender1, DoWorkEventArgs e1)
                {
                    MaterDt = patientService.QueryPatientMaterialDataByParam(this.dtStar.DateTime, this.dtEnd.DateTime);
        ...
        private void DrawChartPie(string Title, string ChartType)
        {
            ...
            var dtMaterial = from p in MaterDetail.AsEnumerable()
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            if (e.Clicks == 2)
            {
                ...
                var hemoId = dtRow["HEMODIALYSIS_ID"].ToString();
                MaterDetail = patientService.QueryPatientMaterialDetailByparam(this.dtStar.DateTime, this.dtEnd.DateTime, hemoId);
                this.gridControl2.DataSource = MaterDetail;
                this.gridView2.ExpandAllGroups();
                if (MaterDetail.Rows.Count > 0)
                {
