...
namespace Hemo.Client.Modules
{
    public partial class PatientMgr : BaseMoudleControl
    {
        ...
        private PatientModel.MED_PATIENTSRow GetPatientRow(DataRow confirmRow)
        {
            ...
            row.PAT_LABEL = confirmRow["PAT_LABEL"].ToString();
            return row;
        }
        /// <summary>
        /// 病人扫描件上传
        ...
        private void tileView1_ItemCustomize(object sender, DevExpress.XtraGrid.Views.Tile.TileViewItemCustomizeEventArgs e)
        {
            ...
            string patlabel= tileView1.GetRowCellValue(e.RowHandle, tileView1.Columns["PAT_LABEL"]).ToString();
            if (string.IsNullOrEmpty(patlabel)) return;

            // 方法1：直接添加到现有元素中
            // 查找照片元素的位置索引（通常是第一个元素）
            if (e.Item.Elements.Count > 0)
            {
                // 创建一个新的文本元素
                DevExpress.XtraGrid.Views.Tile.TileViewItemElement textElement = new DevExpress.XtraGrid.Views.Tile.TileViewItemElement();
                textElement.Column = null; // 不绑定列
                textElement.Text = patlabel;
                textElement.Appearance.Normal.Font = new System.Drawing.Font("微软雅黑", 10F, System.Drawing.FontStyle.Bold);
                textElement.Appearance.Normal.ForeColor = System.Drawing.Color.Red;
                textElement.Appearance.Normal.TextOptions.HAlignment = DevExpress.Utils.HorzAlignment.Center;

                // 设置位置：在照片0区域的底部 Point(0, -10)
                textElement.AnchorElementIndex = 0; // 锚定到照片位置
                textElement.AnchorAlignment = DevExpress.Utils.AnchorAlignment.Bottom;
                textElement.AnchorOffset = new System.Drawing.Point(110, -20); // 向上偏移20像素 向右偏移110像素
                textElement.Height = 25;
                textElement.Width = 80;

                e.Item.Elements.Add(textElement);
            }
        }
