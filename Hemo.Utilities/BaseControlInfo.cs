...
namespace Hemo.Utilities
{
    public class BaseControlInfo
    {
        ...
        public static void BindLookUpEdit(LookUpEdit lookUpEdit, string valueMember, string displayMember, DataTable dataSource, string fieldName, string caption,int defaultSelectedIndex=0)
        {
            ...
            lookUpEdit.Click += (sender, e) =>
            {// 2026-05-21 刘建超：特殊标记，第一次点击时初始化，第二次点击才出来数据
                LookUpEdit lookUp = (LookUpEdit)sender;
                if (lookUp.Tag.Equals("1"))
                {
